...
describe Operation do
	before(:all) do
		...
		@tester.fixtures[@sourceId] = @resource
		@id = @tester.fixtures[@sourceId].id
	end
	before(:each) do
		@tester.fixtures[@sourceId] = @resource.deep_dup
	...
	describe '.get_path' do
		it 'raises an excepion if no url, params, targetId, or sourceId' do
		...
		context 'with operation.targetId' do
			before do
				@operation.targetId = @sourceId
				@tester.fixtures[@sourceId].meta.versionId = @vid
			end
			...
			it 'raises an error if id not present for VREAD' do
				@tester.fixtures[@sourceId].id = nil
			...
			it 'raises an error if vid not present for VREAD' do
				@tester.fixtures[@sourceId].meta.versionId = nil
		...
		context 'with operation.sourceId' do
			...
			it "returns '[type]' path for CREATE" do
				...
				expect(result).to eq("#{@resource_type}")
			end
	...
	describe '.get_payload' do
		...
		it 'returns the body' do
			...
			expect(result).to eq(@tester.fixtures[@sourceId])
		end
	end
