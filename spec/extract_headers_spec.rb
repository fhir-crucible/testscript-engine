...
describe TestScriptRunnable do
  ...
  let(:request_headers) { [FHIR::TestScript::Setup::Action::Operation::RequestHeader.new(field: 'Retry-After', value: '60'), FHIR::TestScript::Setup::Action::Operation::RequestHeader.new(field: 'Location', value: url)]}
  ...
  context '#extract_headers' do
    ...
    context 'given accept, contentType, and a requestHeader' do
      ...
      context 'containing accept and contentType' do
        before do 
          operation.requestHeader = [FHIR::TestScript::Setup::Action::Operation::RequestHeader.new(field: 'Accept', value: 'json'), FHIR::TestScript::Setup::Action::Operation::RequestHeader.new(field: 'Content-Type', value: 'xml')]
        end
      ...
      it 'returns all the denoted headers' do
        expect(runnable.extract_headers(operation)).to eq(accept_pair.merge(content_type_pair).merge(request_headers_pair))
      end 
    end 
  end
end