    ...
    def self.start
      ...
      loop do
        ...
        if input.strip == ''
          ...
          until engine.verify_runnable(input)
...
Would you like to modify this configuration? [Y/N] )
    ...
    def self.modify_configuration
      ...
      input = gets.chomp
      @testscript_path = validate_path(input.strip) unless input.strip == ''

      print 'Set [TESTREPORT OUTPUT DIRECTORY] (press return to skip): '
      input = gets.chomp
      @testreport_path = validate_path(input.strip) unless input.strip == ''
